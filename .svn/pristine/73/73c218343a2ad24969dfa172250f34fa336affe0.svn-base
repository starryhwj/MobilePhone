...
/****** Object:  StoredProcedure [dbo].[AutoCreateMissions]    Script Date: 12/16/2019 11:30:39 ******/
SET ANSI_NULLS ON
GO
...
-- =============================================
ALTER PROCEDURE [dbo].[AutoCreateMissions]
AS
BEGIN
	...
	CASE WHEN CONVERT(varchar(12),A.EndTime,108) = '00:00:00' THEN CONVERT(varchar(11),DATEADD(DAY, 1, GETDATE()),120) + ' ' + CONVERT(varchar(12),A.EndTime,108)
	ELSE CONVERT(varchar(11),GETDATE(),120) + ' ' + CONVERT(varchar(12),A.EndTime,108) END AS EndTime, 
	1 AS Priority , 
	...
	--刷宝任务
	Insert Into [Web_treasuremission]([Status]
	  ,[MobilePhone_id]
      ,[Owner_id]
      ,[CreateTime]
      ,[UpdateTime]
	...
	1 AS Priority , 
	CONVERT(varchar(11),GETDATE(),120) + ' ' + CONVERT(varchar(12),A.StartTime,108) AS StartTime
	From [Web_treasuremissionplan] AS A
	INNER JOIN [Web_missionplantemplate] AS B ON A.MissionPlanTemplate_id = B.id
	INNER JOIN [Web_mobilephone] AS C ON C.MissionPlanTemplate_id = B.id
END
