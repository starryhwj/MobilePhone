...
var KTDatatableDevice = function () {
            ...
            template: function (row) {
                ...
                if (row.Enable == true) {
                    return '<span class="kt-badge ' + status[row.Enable].class +
                        ...
                        '</span> \
                        <button class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="停用" data-enable-id="' + row.id + '">\
                    ...
                    return '<span class="kt-badge ' + status[row.Enable].class +
                        ' kt-badge--inline kt-badge--pill">' + status[row.Enable].title +
                        '</span> \
                        <button class="btn btn-sm btn-clean btn-icon btn-icon-sm" title="启用" data-enable-id="' + row.id + '">\
    ...
    var localSelectorDemo = function () {
        ...
        datatable.on('click', '[data-enable-id]', function () {
            var id = $(this).data('enable-id');
            $.ajax({
                type: "POST",
                url: $("#editdeviceenable_url").text(),
                data: {
                    csrfmiddlewaretoken: document.getElementsByName("csrfmiddlewaretoken")[0].value,
                    id: id
                },
