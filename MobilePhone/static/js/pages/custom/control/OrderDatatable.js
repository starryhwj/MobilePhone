(function ($) {
    ...
    function OrderDatatable(obj) {
        tiggerObj = obj;
        this.initDom();
    }

    //初始化组件DOM元素
    OrderDatatable.prototype.initDom = function () {
        $.ajax({
            type: "POST",
            url: '/Publish/OrderDatatable',
            success: function (response, status, xhr, $form) {
                if (response != 'Error') {
                    tiggerObj.html(response)
                    KTDatatableOrder.init();
                    initSearch();
                }
            }
        })
    ...
    $.fn.orderDatatable = function () {
        var dt = new OrderDatatable(this);
        return dt;
    };

    $.fn.setOrderDatatableAccountID = function (accountid) {
        var datatable = $('#datatable_order').KTDatatable();
    ...
    $.fn.setOrderDatatableAgentID = function (agentid) {
        var datatable = $('#datatable_order').KTDatatable();
        datatable.setDataSourceParam('query', {
            'agentid': agentid
        })
    ...
    $.fn.setOrderDatatableGoodID = function (goodid) {
        var datatable = $('#datatable_order').KTDatatable();
        datatable.setDataSourceParam('query', {
            'goodid': goodid
        })
