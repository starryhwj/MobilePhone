...
/****** Object:  View [dbo].[VAllMissions]    Script Date: 12/27/2019 11:22:08 ******/
SET ANSI_NULLS ON
GO
...
SELECT     '视频任务' AS MissionName, dbo.Web_videomission.id, dbo.Web_videomission.[Status], dbo.Web_videomission.[CreateTime], dbo.Web_videomission.[StartTime], NULL AS [EndTime], 
                      dbo.Web_videomission.[FailReason], dbo.Web_videomission.MobilePhone_id, dbo.Web_tiktokaccount.NickName
...
SELECT     '刷粉任务' AS MissionName, dbo.Web_scanmission.id, dbo.Web_scanmission.[Status], dbo.Web_scanmission.[CreateTime], dbo.Web_scanmission.[StartTime], dbo.Web_scanmission.[EndTime], 
                      dbo.Web_scanmission.[FailReason], dbo.Web_scanmission.MobilePhone_id, dbo.Web_tiktokaccount.NickName
...
FROM         dbo.[Web_watchlivemission] LEFT JOIN
                      ...
                      dbo.Web_tiktokaccount ON dbo.Web_mobilephone.TikTokAccount_id = dbo.Web_tiktokaccount.id
UNION
SELECT     '修改签名任务' AS MissionName, dbo.[Web_changesignaturemission].id, dbo.[Web_changesignaturemission].[Status], dbo.[Web_changesignaturemission].[CreateTime], dbo.[Web_changesignaturemission].[StartTime], 
                      NULL AS [EndTime], dbo.[Web_changesignaturemission].[FailReason], dbo.[Web_changesignaturemission].MobilePhone_id, dbo.Web_tiktokaccount.NickName
